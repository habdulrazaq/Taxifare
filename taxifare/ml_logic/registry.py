...
def save_results(params: dict, metrics: dict) -> None:
    """
    Persist params locally on hard drive at "{LOCAL_REGISTRY_PATH}/params/{current_timestamp}.pickle"
    - if MODEL_TARGET='mlflow', also persist them on mlflow --> unit 03 only
    """
...
def load_model(stage="Production") -> keras.Model:
    ...
    local_models = glob.glob(f"{local_model_directory}/*")
    if not local_models:
        return None
    most_recent_model_path_on_disk = sorted(local_models)[-1]
